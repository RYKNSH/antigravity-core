...
### 0-2. Knowledge Preload & Pre-Flight Rules

過去の学習データをタスク計画に反映し、**ターミナルハングや無限ループを防止するための必須ルール (MR-08, MR-25)** を直ちにロードする。
...
# 必須メタールール (MR-08 / MR-25) の読み込みと警告
ACTIVE_MRS_FILE="$HOME/.antigravity/state/active_mrs.json"
if [ -f "$ACTIVE_MRS_FILE" ]; then
    echo "⚠️  [PRE-FLIGHT CHECK]: Active Metal Rules Loaded!"
...
> [!TIP]
> 過去の失敗パターンに該当するタスクの場合、計画段階で対策を織り込む。特にコマンド実行時のハングを事前予期した代替ツールの検討を行うこと。
...
中間タスクが残っているなら notify_user を呼ばず即着手。
全タスク完了なら宣言形式で報告: 「次は〇〇に着手する」（❌ 選択をユーザーに委ねない）
