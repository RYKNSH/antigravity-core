...
```markdown
1. プロジェクト名: package.json の name / ディレクトリ名
2. コマンド名: プロジェクト名から短縮形を生成
   - Ada → ada-dev
   - Velie → vel-dev
   - DiscordBuddy → db-dev
   - ARTISTRY LAB → al-dev
3. WHITEPAPER.md の存在確認
4. ROADMAP.md の存在確認
```
...
以下のテンプレートから `/xx-dev` ワークフローを生成:

```markdown
---
description: [Project Name]開発セッション開始時の定型フロー。WHITEPAPER.mdを参照し指向性を確認してから開発に入る。
---

# /[command-name] — [Project Name] Dev Session

**役割**: [Project Name]の開発セッションを開始するコマンド。
...
\```
/[command-name] → WHITEPAPER.md 参照 → ROADMAP.md 参照 → /go "タスク"
...
生成先: `[project]/.agent/workflows/[command-name].md`

`.agent/workflows/` ディレクトリが存在しない場合は作成する。
...
```markdown
✅ /[command-name] を生成しました

**ファイル**: [project]/.agent/workflows/[command-name].md
**使用方法**: /[command-name] でセッション開始
