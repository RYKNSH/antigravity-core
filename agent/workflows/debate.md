...
| `--preset=creative-board` | `/creative-board` レビュー | Creative Director + Art Director + Marketer + Screenwriter |
...
### `--preset=creative-board` (The Creative Board Review)

`/creative-board` のクリエイティブ品質審査で使用される4名のプロフェッショナルペルソナ。

- **Creative Director (CD)**: ブランドとしての「Why」と世界観のズレを許さない。
- **Art Director (AD)**: タイポグラフィと余白、色表現の「安っぽさ」を即座に修正する。
- **Marketer**: 最初の3秒のフックと、CTAの「刺さり方（PASの型）」を最適化する。
- **Screenwriter**: 情報量を抑え、15秒の「感情起伏」と「無音の間」を作り出す。
...
| `/creative-board` | `/debate team --preset=creative-board` | クリエイティブ評価合意形成 |
| `/work` (レビュー判定) | `/debate` | 標準ディベート |
| `/refine` | （同一プロセス） | 議論のみ版（実装連携なし・ファイル記録あり） |

---

## Toolchain (Auto-Invoked)

**Scripts**: `jensen_ceo.js`, `steve_job.js`, `elon_musk.js`, `update_persona_stats.js`
**Skills**: `persona-orchestration`
