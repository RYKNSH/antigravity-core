...
|------|------|
| **入力** | なし（自動検出） |
| **出力** | 環境ステータスレポート, state/NEXT_SESSION.md の内容（存在時）, Deferred Tasks リトライ結果 |
...
| **出力** | `state/NEXT_SESSION.md`, ブレインログ, コミット |
...
| 項目 | 定義 |
|------|------|
| **入力** | なし（自動検出）, `quick`(任意), `/fbl` Phase 3.5 経由フラグ(内部) |
| **出力** | UX Performance Audit Score Card（Lighthouse 4カテゴリ + CWV + axe-core violations）, Verdict |
| **完了条件** | Phase 4 Audit Report 出力完了。quick時: Phase 0 + 1 + 4 のみ |
...
| 条件 | 動作 |
|------|------|
| `/checkout` 完了 | state/NEXT_SESSION.md に次タスクを記録 → 次回 `/checkin` で自動再開 |
...
# RAM量からSWAP閾値を動的算出（RAM の 12.5% = 1/8）
# 16GB → 2048MB, 8GB → 1024MB, 32GB → 4096MB
ram_bytes=$(sysctl -n hw.memsize 2>/dev/null || echo 17179869184)
swap_threshold_mb=$((ram_bytes / 1024 / 1024 / 8))
echo "🏥 Health Check: SWAP ${swap_mb}MB (threshold: ${swap_threshold_mb}MB, RAM: $((ram_bytes / 1024 / 1024))MB)"

# 閾値超過時のみクリーンアップ
if [ $(echo "$swap_mb > $swap_threshold_mb" | bc) -eq 1 ]; then
...
- ~~SWAP閾値のマシン別動的化~~ → ✅ 実装済み（RAM 12.5% 比率ベース）
...
3. **削除タイミング**: `/checkout` 完了時に削除（state/NEXT_SESSION.md に引き継ぎ）
