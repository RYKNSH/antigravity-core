...
| **`/ad`** | ad.md | 「広告作る」「動画書き出す」「クリエイティブレビュー」<br>`/ad brief` / `/ad review` / `/ad build` のサブコマンド体系<br>旧 `/creative-board` / `/creative-pipeline` を統合 |
...
| コマンド | WF | 備考 |
|---------|-----|---------|
| `/core-run` | core-run.md | Daemon CoreのCLIゲートウェイ。タスクをキューPush→Daemon自律実行<br>`--status` / `--list` / `--priority high\|normal` |
| `/daemon-dev` | daemon-dev.md | Daemon Core開発セッション開始フロー |
| `/core-dev` | core-dev.md | Antigravity Core開発セッション（Phase 8専用） |
| `/core-ci-dev` | core-ci-dev.md | Antigravity Core Self-Improving Pipeline開発セッション |
...
## 🚨 System Alerts & Self-Healing (テレメトリ)

バックグラウンドプロセス（Git Hooks、Watchdogs等）は、致命的なエラー発生時に `~/.antigravity/state/SYSTEM_ALERTS.md` へテレメトリを出力します。
`/checkin` および `/go` (Phase 0) の実行時にこのファイルが読み込まれ、AIに非同期エラーとして通知されます。AIはこれを検知した場合、自己修復タスクを実施し、完了後に当該ファイルをクリアする必要があります。
