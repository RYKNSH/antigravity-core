...
### Approach Category Tracking（同カテゴリの再使用禁止）

試行済みアプローチをカテゴリ分類し、**次の試行で同カテゴリを禁止**:

| カテゴリ | 例 |
|---------|---|
| **Code-Level Fix** | 変数修正、条件分岐追加、型変更、引数修正 |
| **Architecture Shift** | モジュール分離、レイヤー変更、責務移動、依存方向の反転 |
| **Dependency Change** | ライブラリ変更/追加、バージョン変更、別ツール導入 |
| **Design Rethink** | データモデル変更、API設計変更、状態管理パターン変更 |
| **Assumption Inversion** | 前提を逆転（同期→非同期、プル→プッシュ、クライアント→サーバー） |

**カテゴリ強制ルール**:
- 試行1 → 自由選択。カテゴリを `.session_state` に記録
- 試行2 → **試行1と別カテゴリ必須**
- 試行3 → **Assumption Inversion 強制**（第一原理から全前提を洗い出し、最も疑わしい前提を逆転）

> [!IMPORTANT]
> 3回失敗したということは「前提自体が間違っている」可能性が極めて高い。
> 試行3で Assumption Inversion を強制するのは、「コードでも設計でもなく、前提を直す」ためである。
...
| # | カテゴリ | 内容 | 結果 |
|---|---------|------|------|
| 1 | [Code-Level Fix] | [修正内容] | 失敗 |
| 2 | [Architecture Shift] | [修正内容] | 失敗 |

### 今回の試行カテゴリ（上記と重複不可）
- カテゴリ: [Design Rethink / Assumption Inversion]
