...
> [!CAUTION]
> **Phase 3 ゲート**: Phase 2 の「再発防止ソリューション」に書いた内容を、Phase 3 でファイル変更として実装するまで Phase 4 に進むな。
> 「意識する」「次回気をつける」は改善ではない。構造を変えろ。
> ワークフロー/スキル/ルールのいずれかに diff が出ていなければ、Phase 3 未完了とみなす。
...
2. **実装**: ユーザー承認後、即座に実装・適用する。
    ...
    - ルール更新
3. **検証**: 実装内容が正しいか確認。
4. **コミット**: 改善内容を `kaizen: [内容]` プレフィックスでコミット。

**ゲート条件（Phase 4 進行の前提）:**
- [ ] Phase 2 で特定した課題に対する具体的なファイル変更がある
- [ ] `kaizen:` プレフィックスのコミットが存在する
- [ ] 「意識する」「次回やる」ではなく、構造的な変更である

**フィードバックループ:**
```
チェックアウト自己評価 → 課題特定 → ソリューション実装(必須) 
    → kaizen コミット → Phase 4 へ
```
