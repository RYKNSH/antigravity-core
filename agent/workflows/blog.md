...
# /blog - Spokesperson & Publishing System

**役割**: プロジェクトの専属広報官。作業ログからの記事生成、配信、およびユーザーの修正からの学習を一括管理する。

## コマンドバリエーション
- `/blog` (デフォルト): 記事/Daily Logの生成（Draft作成）
- `/blog publish`: 生成されたDraftの昇格と配信予約
- `/blog learn <URL>`: 修正済み記事からのトーン学習と最終配信設定
...
## Module: Publish & Distribute (`/blog publish`)

1. **Promote Drafts**: `node $ANTIGRAVITY_DIR/agent/scripts/promote_drafts.js`
2. **Smart Scheduling**: `node $ANTIGRAVITY_DIR/agent/scripts/schedule_posts.js`
   - 重複を避けて直近のゴールデンタイム（08, 12, 18, 22時）に予約
3. **GAS Deployment**: `cat $ANTIGRAVITY_DIR/agent/scripts/gas_publisher.js | pbcopy`
   - 配信処理はGAS側が担当（トリガー確認用）

---

## Module: Active Learning (`/blog learn <URL>`)

1. **正解テキスト取得**: `node $ANTIGRAVITY_DIR/agent/scripts/fetch_notion_page.js <PAGE_ID>`
2. **差分分析と学習**: 現在のスキル (`checkpoint_social_blog.md`) と差分を比較し、AIの執筆スキル（トーン・比喩・NGワード）をユーザーの好みに合わせて微修正
3. **Smart Scheduling**: `node $ANTIGRAVITY_DIR/agent/scripts/schedule_posts.js` を再実行して Ready 状態にする
4. **Social Sharing**: 記事の核心を突く他SNS（X, Threads）用の投稿文を3パターン生成して提案

---

## Toolchain

**Scripts**: `auth_notion.js`, `session_state.js`, `notion_poster.js`, `generate_social_assets.js`, `promote_drafts.js`, `schedule_posts.js`, `gas_publisher.js`, `fetch_notion_page.js`
**Skills**: `frontend-design`
**Knowledge**: `social_knowledge`, `soloprostudio_social_knowledge_ecosystem`, `artistory_studio`, `portable_studio_app`
