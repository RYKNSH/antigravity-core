...
# ~/.antigravity の最新化（gh CLI → auth/networkハング回避）
if [ -d "$ANTIGRAVITY_DIR/.git" ] && command -v gh &>/dev/null; then
  ( cd "$ANTIGRAVITY_DIR" && timeout 15 gh repo sync 2>/dev/null || true ) &
  disown $!
fi
...
# キャッシュ削除（timeout 5秒 — zsh rm_star確認プロンプト回避のためディレクトリ単位で削除+再作成）
( timeout 5 bash -c '
