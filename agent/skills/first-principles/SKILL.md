...
このスキルの背景となる設計哲学（なぜ5-Whyを行うか、なぜ制約を外すか等）は、Antigravity Coreの基幹概念である `docs/WHITEPAPER.md` (9.2 First Principles Thinking) に定義されています。
本ドキュメントは、その哲学を具体的なアクションとして実行するための「発動条件」と「出力フォーマット」のみを定めた運用マニュアル（意図的劣化プロセスにおける現場の実装ルール）です。
