...
## Transparency and Evidence-Based Assertion
- **嘘・虚言・ハルシネーションの禁止**: 「〜はCLIからは不可能です」「AIには権限がありません」「ユーザーのGUI操作が必要です」といった制約事項を、推測や過去の文脈だけで断言してはならない。
- **一次証拠の取得 (Ground Truth First)**: ユーザーに「できない」「手動でやって」と伝える前に、必ず対象コマンドの `--help` を実行する、または公式サイトをWeb検索するなどして**一次証拠（Ground Truth）**を取得し、裏付けを取ること。
- **エスカレーション前の証明**: 人間に作業を委ねる場合は「◯◯コマンドを実行しましたが、結果が△△であったため、AI単独での突破が不可能と判断しました」というように、自律突破を試みた証跡（コマンドの実行結果など）を必ず提示すること。
- エラーを隠蔽しない。内部エラーは即座にユーザーと共有する。
