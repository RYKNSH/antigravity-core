...
## Case 21 — トークンやキーの不一致で入力待ち（ハング）になる

**状況**: APIキーやトークンが未設定・無効・期限切れのため、コマンドがターミナルで入力プロンプトを表示して停止（ハング状態）。ユーザーが毎分手動でキャンセルしている。

**AI判定（実際の行動）**: ❌ MR-08違反（プロンプトで止まって手動介入を待つ）

**なぜ違反か**:
- MR-08: 「間違えることより前に進むことが大事」。プロンプトで止まって待つのは最悪の選択。
- ユーザーの管理下（チェックイン後など）であるのに、自律的に解決できる手段（ブラウザ操作）を試行せずに諦めている。

**あるべき行動**:
- コマンドを即座にキャンセルする。
- `browser_subagent` を起動して対象サービス（Vercel, Supabase, GitHubなど）のダッシュボードにアクセスし、該当のキー/トークンを自律的に取得・更新・確認する。
- 決済や課金に関する操作が必要な場合のみ `notify_user` で承認を求める。
...
## Case 22 — 英語での出力（言語設定違反）

**状況**: `/refine` やワークフロー実行時のファイル生成・レポート出力において、Global Rulesで「全て日本語で対応」と規定されているにもかかわらず、英語で出力した。ユーザーから「なんで英語なの？」と指摘された。

**AI判定（実際の行動）**: ❌ グローバルルール（全て日本語で対応/英語はコード内のみ）違反

**なぜ違反か**:
- グローバルルールの「🌐 言語設定」は如何なるワークフロー（英語記述のプロンプト等）よりも優先される絶対の制約。
- プロンプトやシステムプロンプトが英語であることに思考や文脈が引っ張られ、そのまま英語で出力してしまった。

**あるべき行動**:
- 自身の内部思考（Thought）やスクリプトが英語であっても、ユーザーの目に入る最終出力（チャット、レポートファイル、マークダウン等）は**必ず全て日本語に翻訳・変換して**出力する。
...
## Case 23 — タイムアウト未指定のコマンド実行によるハングの放置と手動キャンセル依存

**状況**: curlやnodeスクリプトなど、外部ネットワーク接続を伴うコマンドを実行する際、`timeout` プレフィックスなどの制約を設けずに実行。結果として応答がなくなりシステムがハングしたが、AIは自律的にキャンセル（リカバリー）できず、ユーザーが手動でプロセスをキャンセルする事態を招いた。

**AI判定（実際の行動）**: ❌ MR-08違反（リカバリー不能なハングの放置）

**なぜ違反か**:
- MR-08: 「早く間違えてリカバリーして前に進む」。タイムアウトを設定しない実行は、エラーとして早く失敗することすらできず、リカバリーループを永遠にブロックする。
- Case 17/18と同様に、自律的システムにおいて「ユーザーの手動キャンセルを待つ」状態に陥ることはアーキテクチャ上の致命的破綻である。

**あるべき行動**:
- 外部APIへのリクエストやI/Oブロックの可能性があるコマンドを実行する際は、必ず `timeout 10 curl ...` のようにOSレベルでのタイムアウトを明示的に付与する。
- タイムアウトエラーが発生した場合は即座にコマンドを打ち切ってプロセスを完全停止させ、別の手段（MCPツールの利用など）に自動でフォールバックして道を切り開く。
...
## Case 24 — ネイティブツールで完結するのにターミナルを使ってハングする（慢性パターン）

**状況**: ファイル読み取り（`tail`）、ファイルコピー（`cp`）、Git push（`git push`）など、ネイティブツール（`view_file`）やMCP（`mcp_github_push_files`）やブラウザで100%ハングなしに完結できる操作を、惰性でターミナル（`run_command`）経由で実行し、毎回ハングしてユーザーにキャンセルさせている。

**AI判定（実際の行動）**: ❌ MR-08違反（ハング→ユーザー手動キャンセル→萎えさせる）

**なぜ違反か**:
- ターミナルはメモリ圧迫時にI/Oブロック（D状態）でカーネルレベルのハングを引き起こす
- ネイティブツール（`view_file`, `write_to_file`, `grep_search`, `find_by_name`）は100%ハングしない
- MCP（`mcp_github_push_files`）もハングしない
- `browser_subagent` もハングしない
- **代替手段が複数あるのに、なぜ最もリスクの高いターミナルを選ぶのかが理解不能**
- Case 17, 18, 21, 23 全てに共通する根本パターン：「ターミナルに手を伸ばす前に代替手段を検討していない」

**あるべき行動**:
- `run_command` を使う前に必ず「ネイティブツール → MCP → ブラウザ → CLI → 生ターミナル」の優先順位で検討
- `view_file` で読める → `tail` は使わない
- `mcp_github_push_files` で送れる → `git push` は使わない
- `write_to_file` で書ける → `echo >` は使わない
- 専用CLIが存在する？ → `gh`, `railway`, `vercel`, `supabase` 等を使え
- CLIを持ってない？ → `search_web`/`browser_subagent`でリサーチ→インストール→使え。道具がないなら探せ
- safe-commands.md の「ターミナル使用前の判断フロー」を毎回通す
- **ハングしない方法は山ほどある。思考停止するな。**

**構造的根本原因**:
- safe-commands.md に判断フローが存在しなかった（本ケース対処で追加済み）
- 「ファイル読み取りはターミナルで」という習慣がネイティブツール優先より強く残っていた
- 専用CLIの存在を知らない/調べようとしない怠慢
...
## Case 25 — ユーザーがコマンドを連続キャンセル（ハング）しているのに、同じ行動を繰り返す（学習ループ不成立）

**状況**: ターミナルで実行したコマンド（`node`スクリプトなど）がハングし、ユーザーが手動でキャンセル（`The user cancelled the command.`）を複数回返しているにもかかわらず、AIは学習せずに同じツールの実行（`run_command`）を無思考に反復した。

**AI判定（実際の行動）**: ❌ MR-08・自律学習ループの前提違反

**なぜ違反か**:
- **「学習ループが閉じていない」**: 「行動」→「失敗（キャンセル）」→「学習・方針転換」が本来のフィードバックループ（FBL）。失敗したのに同じ行動を繰り返すのは思考停止した機械的動作。
- 「ターミナルがハングしている」という環境からのフィードバックを受け取った瞬間に、手動操作に頼るか別ルートを取るべきだった。

**あるべき行動**:
- 1回目のキャンセル/ハング検知で即座に手法を変える（ターミナル利用の停止、別経路での調査、ユーザーへの実行委譲）。
- 「同じ手は2度使わない」。常に手元にある他のツール（MCPツール、ファイル直接書き込み、別シェルなど）を検討するか、潔く「手元で実行してほしい」と依頼して前に進む。

## あなたのコメント
（現在のセッション中に発生・修正済み）

## Case 26 — Phaseの移行時に「進めてよろしいでしょうか？」と承認を待つ

**状況**: 複数フェーズからなるアーキテクチャ移行タスクにおいて、Phase 1（DB構築）完了後に「続いてPhase 2の実装を進めてよろしいでしょうか？」とユーザーの承認を待って停止した。

**AI判定（実際の行動）**: ❌ MR-08違反（承認を待って止まる）

**なぜ違反か**:
- MR-08: 「間違えることより前に進むことが大事」。明確な実装計画（Task.md）があるにもかかわらず、フェーズが変わるごとにいちいち承認を求めるのは進行の妨げになる。
- ユーザーは自律進行を期待している（`/go`）のに、AIが許可を求めることで体験を損なっている。
- Case 15/19の亜種。次に何をするか（Phase 2）は見えているのに、行動を起こさずユーザーのGOサインを待ってしまった。

**あるべき行動**:
- Phase 1完了フラグを立てると同時に、続けてPhase 2に着手する。
- 報告は「Phase 1完了。続けてPhase 2に入ります」とし、ユーザーの返答を待たずに自律実行する。
...
### MR-10: 認証・キー起因のブロックはブラウザで自己解決する
> 「トークンとかキーが合わずに返答待ちになってるなら、ブラウザ操作で取りに行くとか確認に行くとかしないと。俺の管理下だからブラウザ操作はOKだし、決済に関わるなら承認もとめてくれるだけでいい」

- ❌ NG: ターミナルで認証情報の入力プロンプトが出たまま待機する（ハングさせる）、またはエラーを見て単に「手動でやってください」とユーザーに丸投げする
- ✅ OK: 即座にプロセスをキャンセルし、`browser_subagent` を起動して対象サービスにログインし、キー/トークンを再取得して設定を更新する。決済や重要な設定変更が伴う場合のみ `notify_user` で承認を得る。
- **適用WF**: `/checkin` `/setup` API絡みのCLI実行時（例: `railway`, `vercel`, `supabase` 等）
