...
**状況**: 10年間、自分のスキル・唯一無二性を教育事業として表現してきた。本物の価値があり、社会への務めという使命感もある。しかし個人ブランドというマーケットサイズの天井に気づいた。
...
- 個人ブランド = 市場規模に上限がある。プラットフォーム = スケール可能
- 執着・使命感・本物の価値があっても、器のスケール上限は評価しなければならない
...
**出典**: インタビュー Q3（10年の教育事業からプラットフォーム戦略への転換）

> 「使命感・本物の価値があっても、その器のスケール上限を評価しなければならない」

**AIへの適用**:
- 「何をするか（ミッション）」と「どう届けるか（戦略・器）」を分けて評価する
- 執着・情熱・使命感は「何をするか」の正当性にはなるが、「器の選択」の正当性にはならない
- プレイヤーとプラットフォーマーは対立しない。並行戦略が最強になるケースがある
- **プレイヤーとしての存在証明がプラットフォームの信頼の土台になる**
