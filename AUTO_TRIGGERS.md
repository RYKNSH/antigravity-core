...
- **トリガー**: ユーザーが「慎重にやって」「手動で確認して」「一つずつ」等の慎重モードを要求した時
  - **アクション**: `/level 1` (Supervised)
  - **目的**: 破壊的操作や重要な判断をユーザー確認付きで進めるため。

- **トリガー**: ユーザーが「全自動で」「止まらないで」「勝手にやって」等の全自動モードを要求した時
  - **アクション**: `/level 3` (Full Auto — 確認付き)
  - **目的**: 信頼関係確立後の完全自律実行。
...
## 4.5 真実検証 (Galileo Test)

- **トリガー**: ユーザーが「本当にそう？」「常識では」「一般的に〜と言われている」「検証して」「ファクトチェック」等の検証要求をした時
  - **アクション**: `/galileo`
  - **目的**: 多数派のコンセンサスに流されず、一次ソースに基づいて真実を検証するため。

- **トリガー**: `/debate` や `/spec` の議論中に「〜が常識」「〜が一般的」「みんな使っている」等のソースなき主張が根拠として出た時
  - **アクション**: `/galileo quick [その主張]` を提案
  - **目的**: 根拠のない「常識」をファクトチェックし、議論の質を高めるため。
...
## 7. Smart Triggers（AI自律判断）

> [!IMPORTANT]
> セクション1-6は「ユーザーの発言」がトリガー。
> セクション7は「AIが状況を分析して自律的に判断」するトリガー。

- **トリガー**: 実装完了（コード修正が一段落）
  - **アクション**: `/verify` 自動実行（Phase 0 で規模自動判定）
  - **目的**: ユーザーが「検証して」と言わなくても品質を自動担保

- **トリガー**: 変更ファイル数が6以上 or 新APIエンドポイント追加
  - **アクション**: `/verify` を deep レベルで自動選択
  - **目的**: 大規模変更には深い検証を自動適用

- **トリガー**: verify 通過 + staging環境が存在
  - **アクション**: `/ship staging` を提案
  - **目的**: 検証完了からデプロイへのスムーズな遷移

- **トリガー**: 新しいAPIエンドポイントが追加された
  - **アクション**: `/error-sweep` の Contract Verification（Phase 4）を重点チェック
  - **目的**: API型契約の不整合を早期検出

- **トリガー**: アーキテクチャ変更を伴う実装（DB schema変更、新ディレクトリ構造の作成等）
  - **アクション**: `/galileo` を自動組み込み（設計判断の検証）
  - **目的**: 「みんなやってるから」ではなく証拠ベースで設計判断
